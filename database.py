...
class Database:
    ...
    def put_entry(self, entry):
        ...
        self.cursor.execute(
            "INSERT INTO RSS_ENTRIES(entry_title, entry_publication_date, entry_summary, entry_link, was_already_posted)\
                            VALUES(?, ?, ?, ?, 0)",
            (entry.title, timestamp, entry.summary, entry.link),
        )
    ...
    def get_latest_headline(self):
        ...
        self.cursor.execute("""
            ...
            LIMIT
                1
        """)
        post_id, post_title, post_date, post_summary, post_link = (
    ...
    def mark_headline_as_read(self):
        ...
        self.cursor.execute(
            "SELECT entry_id FROM RSS_Entries ORDER BY entry_publication_date DESC LIMIT 1"
        )
